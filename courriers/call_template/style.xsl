	...
	<xsl:template name="generalStyle">
		<style>
			...
			h4{line-height: 0.2em}
                        p{font-size:15px;}
@media print {
				#logoContainer, #print-button, #letter_name, #lastFooter, #contactUs, #myAccount {
				display:none;
					}
				body {
				font-size:80%;
...
<xsl:template name="generalStyle-sans-hover">
		<style>
			body {@@language_related_body_css@@ background-color:#fff}
			.listing td {border-bottom: 1px solid #eee}
			.listing th {background-color:#f5f5f5 }
...
@media print {
	...
	<xsl:template name="bodyStyleCss">
		font-family: arial; color:#333; margin:0; padding:1em; font-size:80% 
	</xsl:template>
